...
if (get_field('bitrix_webhook', 'options')) {
    function from_cf_to_bitrix_sender($contact_form)
    {
        ...
        // Основная форма заявки с главной страницы
        ...
        if ('Тест-драйв' == $title) {
            ...
            $crm_deal_add_query_data = http_build_query([
                'fields' => [
                    'TITLE'             => get_field('bitrix_deal_title', 'options') . ' [ТЕСТ-ДРАЙВ]',
                    'UF_CRM_1615817578' => 221,
                    'CATEGORY_ID'       => 71,
                    'STAGE_ID'          => 'C71:NEW',
                    'UF_CRM_1612504156' => 2909,
                    ...
                    'COMMENTS'          => "Дата тест-драйва: " . $posted_data[ 'form-date' ]
                        . " Время тест-драйва " . $posted_data[ 'form-range-value' ]
                        . " Ждет звонка: " . $recall,
                    'CONTACT_ID'        => $contact_result[ 'result' ],
                ],
                'params' => [
        ...
        if ('Запись на сервис' == $title) {
            ...
            $crm_deal_add_query_data = http_build_query([
                'fields' => [
                    'TITLE'             => get_field('bitrix_deal_title', 'options') . ' [СЕРВИСНОЕ ОБСЛУЖИВАНИЕ]',
                    'UF_CRM_1615817578' => 221,
                    'CATEGORY_ID'       => 71,
                    'STAGE_ID'          => 'C71:NEW',
                    'UF_CRM_1612504156' => 2909,
                    ...
                    'UF_CRM_1634038284' => get_field('bitrix_deal_site', 'options'),
                    'UF_CRM_1611565554' => (int) $form_model,
                    'COMMENTS'          => "VIN: " . $posted_data[ 'form-vin' ]
                        . " Год производства: " . $posted_data[ 'form-year' ]
                        . " Дополнительно: " . $posted_data[ 'form-message' ],
                    'CONTACT_ID'        => $contact_result[ 'result' ],
                ],
                'params' => [
                    'REGISTER_SONET_EVENT' => 'Y'
        ...
        // Форма заявки со страницы определенного аксессуара
        if ('Заявка по определенному аксессуару' == $title) {
            $submission = WPCF7_Submission::get_instance();
            $posted_data = $submission->get_posted_data();
            // Далее перехватываем введенные данные в полях Contact Form 7:
            $name = $posted_data[ 'form-name' ];
            ...
            $crm_deal_add_query_data = http_build_query([
                'fields' => [
                    'TITLE'             => get_field('bitrix_deal_title', 'options') . $posted_data[ 'hidden-input-title' ],
                    'UF_CRM_1615817578' => 221,
                    'CATEGORY_ID'       => 71,
                    'STAGE_ID'          => 'C71:NEW',
                    'UF_CRM_1612504156' => 2909,
